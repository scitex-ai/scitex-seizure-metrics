...
#
# WHY a layered install (not a bare PYTHONPATH=src trick): the shared ci-cpu.sif
# bakes scitex-dev[all,dev] DEPS, NOT this package's. So we install THIS checkout
# + its [all,dev] extras (WITH dependency resolution) into a writable --target
# dir and prepend that on PYTHONPATH. The SIF still supplies the heavy shared
# base (pip/uv, the python interpreters, scitex-dev's deps), so only this
# package's own thin dep set is fetched per run.
#
...
# + ephemeral and unique-per-run (RUN_ID/ATTEMPT) + per-version-isolated so
# concurrent matrix legs and re-runs never collide.
...
# matplotlib and RACE to build fontList.json in that shared dir. One stable dir
...
# Install this package + its [all,dev] extras WITH deps into the writable target.
...
# Parallelise with pytest-xdist when present. Each xdist worker is a SEPARATE
# PROCESS, so matplotlib's global rcParams / pyplot state are naturally isolated
# per worker — the safe way to parallelise a matplotlib-touching suite.
#
# Worker count: use ALL cores (each matrix leg runs on its own dedicated
# self-hosted node, one runner per node, so there is no co-tenant to yield half
# the box to). Floor 4. pyproject addopts carries `-v`; override to `-q` here.
...
# workers. matplotlib may not be a dependency of this package; only warm the
# font cache when it's importable (no-op otherwise — never fail the run on an
# optional warm-up).
...
# pytest-xdist may not be installed (it ships in scitex-dev[dev], pulled in via
# the [all,dev] install above). Detect it; fall back to single-process if absent
# so CI never dies on a missing -n plugin.
XDIST_ARGS=()
if python -c "import xdist" 2>/dev/null; then
    # `--dist load` (per-TEST round-robin) spreads parametrized cases across ALL
    # workers; loadscope would pin a whole module to one worker.
...
# cores but YIELDS the CPU and disk to any higher-priority process. exec
# replaces the shell so signals/exit code propagate to the runner step.
