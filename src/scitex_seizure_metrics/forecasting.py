...
- evaluate(alarms, seizures, policy, ...) — accepts pre-computed alarm times; useful when alarms come from a different pipeline.
...
- sweep_policies — same metrics across multiple AlarmPolicy values (e.g. cadence ablation).
...
def evaluate(
    ...
    *,
    ...
    if alarms.size > 0:
        tiw_seconds = _alarm.union_length(
            np.column_stack(
                [
    ...
    for _ in range(n_surrogate):
        rand_alarms = surro_fn(
            int(alarms.size),
            max(total_recording_time - policy.sop_seconds - policy.sph_seconds, 1.0),
            rng,
        )
        sc_s, _ = _alarm.alarm_match(
            rand_alarms, seizures, policy.sph_seconds, policy.sop_seconds
        )
    ...
    rep = MetricsReport(
        ...
        regime="forecasting",
        sensitivity=sens,
        n_ref_events=int(seizures.size),
        n_tp=tp,
        n_fp=fp,
        fp_per_hour=fp_per_hour,
        fp_per_day=fp_per_hour * 24.0,
        sph_seconds=policy.sph_seconds,
        sop_seconds=policy.sop_seconds,
...
def bootstrap_ci(values, n_boot: int = 1000, ci: float = 0.95, rng_seed: int = 0):
