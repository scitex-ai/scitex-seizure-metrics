...
description: |
  ...
  [HOW] `from epileval import detection, forecasting, AlarmPolicy` — call `detection.evaluate(y_true, y_proba)` for sample-based, `forecasting.evaluate_stream(proba, times, seizures, AlarmPolicy(...))` for alarm-based, and `bridge.sample_to_alarm(...)` when only one regime was published.
...
`epileval` lets one method be reported on every prior paper's exact
...
* [01_installation.md](01_installation.md) — install epileval and its `[plots]` / `[test]` / `[dev]` / `[docs]` / `[all]` extras; umbrella `pip install scitex[epileval]`.
...
```python
import scitex
scitex.epileval.detection  # same object as epileval.detection
```
