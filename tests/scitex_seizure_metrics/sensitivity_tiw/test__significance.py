"""Tests for ``sensitivity_tiw._significance`` — above-chance tests.

Covers ``chance_sensitivity`` (the diagonal), ``binomial_above_chance``
(exact one-sided binomial + Wilson CI), ``surrogate_above_chance``
(circular time-shift permutation), and the ``TiWSignificance`` container.

Synthetic data with KNOWN ground truth: a better-than-chance predictor
is provably significant; a random predictor is not.
"""
...
# --------------------------------------------------------------------------
# Synthetic-data builders (KNOWN ground truth)
# --------------------------------------------------------------------------
