...
correct. Generated artefacts are saved to /tmp/epileval-plots/ for
manual inspection.
"""
...
from epileval import AlarmPolicy, forecasting, plots


PLOT_DIR = "/tmp/epileval-plots"
...
def operating_curve():
    ...
    pol = AlarmPolicy(sph_seconds=300, sop_seconds=600, cadence_seconds=60,
                      refractory_seconds=600, alarm_threshold=0.5)
    return forecasting.sweep_thresholds(
        proba, times, seizures, pol,
        thresholds=np.linspace(0.05, 0.95, 19),
        total_recording_time=21600.0, n_surrogate=200,
    )


def test_plot_sensitivity_vs_fp_per_hour(operating_curve):
    ...
    fig.savefig(f"{PLOT_DIR}/sensitivity_vs_fp_per_hour.png", dpi=110,
                bbox_inches="tight")
    plt.close(fig)
...
def test_plot_ioc_vs_surrogate(operating_curve):
    ...
    fig.savefig(f"{PLOT_DIR}/ioc_vs_surrogate.png", dpi=110,
                bbox_inches="tight")
    plt.close(fig)
...
def test_plot_sample_vs_alarm_scatter():
    ...
    fig.savefig(f"{PLOT_DIR}/sample_vs_alarm_scatter.png", dpi=110,
                bbox_inches="tight")
    plt.close(fig)
...
def test_plot_cadence_ablation():
    ...
    policies = [AlarmPolicy(sph_seconds=0, sop_seconds=600,
    ...
    fig.savefig(f"{PLOT_DIR}/cadence_ablation.png", dpi=110,
                bbox_inches="tight")
    plt.close(fig)
...
def test_plot_reliability_diagram():
    ...
    from epileval import calibration
    cal = calibration.calibration_report(y, p, n_bins=10)
    fig, ax = plots.reliability_diagram(cal)
    fig.savefig(f"{PLOT_DIR}/reliability_diagram.png", dpi=110,
                bbox_inches="tight")
    plt.close(fig)
...
def test_plot_metric_correlation_heatmap():
    ...
    fig.savefig(f"{PLOT_DIR}/metric_correlation_heatmap.png", dpi=110,
                bbox_inches="tight")
    plt.close(fig)
